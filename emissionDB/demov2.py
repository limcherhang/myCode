...
def getEmissionAsset(sId: str, cursor: pymysql.cursors.Cursor):
    sqlCommand = f"""
        SELECT EFId, companyId, sourceOfEmission, vehicleType, fuelEfficiency, co2Factor, ch4Factor, n2oFactor, ar4Factor, ar5Factor, baseUnit, source, urlName, sheetName, file, link, year, emissionUnit, ch4Unit, n2oUnit, sourceType FROM emissionFactor.emissionAsset
        WHERE sId='{sId}'
    """
...
if __name__ == '__main__':
    ...
    conn = MySQLConn(config['mysql_azureV2mysql_azureV2mysql_azureV2'])
    ...
    with conn.cursor() as cursor:
        ...
        for sId, _id, _type, scope, typeOfEmission, category, categoryKey, nameKey, sourceType, usedRegion, sourceOfEmission, EFSId, languageId, sourceName in MongoInfo:
            ...
            methods = []
            for EFId, companyId, _sourceOfEmission, vehicleType, fuelEfficiency, co2Factor, ch4Factor, n2oFactor, ar4Factor, ar5Factor, baseUnit, _source, urlName, sheetName, _file, link, year, emissionUnit, ch4Unit, n2oUnit, sourceType in emissionAsset:
                method = {}
                if companyId:
