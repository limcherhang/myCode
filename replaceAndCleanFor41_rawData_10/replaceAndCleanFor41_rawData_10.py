...
def process_delete(schema: str, table: str, start: datetime.datetime, end: datetime.datetime, cursor: pymysql.cursors.Cursor):
    while True:
        ...
        try:
            ...
            logger.info(f"DELETE mysql_fn {table} from {start} to {end} Succeed!")
            
        except Exception as ex:
            logger.error(f"Error executing {delete_sql} to mysql_fn")
...
if __name__ == '__main__':
    ...
    year = nowTime.year
    process_date = datetime.datetime(2023,11,1)# nowTime - datetime.timedelta(days=1) # 昨天的日期，昨天以前的都要replace和清掉
                    ...
                    if "API" in table and table != 'RESTAPI':
                        ...
                        if errorCode == 1:
                            ...
                            if exist_data == ():
                                ...
                                logger.error(f"It still have some data doesn't move to dataPlatform{year}")
                        ...
                        replace_sql = f"""
                            REPLACE INTO {schema}{write_year}.{table}_{write_month}
                            (
                                SELECT * FROM {schema}.{table} WHERE DBts >= '{date}' AND DBts < '{next_time}' AND GWts >= '{date}' AND GWts < '{next_time}'
                                EXCEPT
                                SELECT * FROM {schema}{write_year}.{table}_{write_month} WHERE DBts >= '{date}' AND DBts < '{next_time}' AND GWts >= '{date}' AND GWts < '{next_time}'
                            )
                        """
                        try:
                            logger.debug(replace_sql)
                        ...
                        if errorCode == 1:
                            ...
                            if table == 'ZB':
                                sqlCommand = f"""
                                    SELECT * FROM {schema}.{table} WHERE DBts >= '{date}' AND DBts < '{next_time}' AND GWts >= '{date}' AND GWts < '{next_time}' GROUP BY DBts, GWts, ZBts, ieee, clusterID
                                    EXCEPT
                                    SELECT * FROM {schema}{write_year}.{table}_{write_month} WHERE DBts >= '{date}' AND DBts < '{next_time}' AND GWts >= '{date}' AND GWts < '{next_time}'
                                """
                            else:
                            ...
                            if exist_data == ():
                                ...
                                logger.error(f"It still have some data doesn't move to {schema}{year}")
                                logger.error(exist_data)
                        del errorCode
                    date = next_time
